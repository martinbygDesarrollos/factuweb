...
$emailTo = "desarrollo@gargano.com.uy,soporte@byg.uy";
...
$resultado = file_get_contents(URL_REST.'reports/status?Estado=ER&Desde='.$fechadesde.'&Hasta='.$fechahasta, false, $contexto);
...
if ( isset($resultadoJson) ){
	...
	if ( $resultadoJson->resultado->codigo == 200 ){
		$tabla = "<table><theader><tr><td>RUT</td><td>Emisor</td><td>Receptor</td><td>Período</td><td>Estado</td><td>Fecha recepción</td></tr></theader>";
		$tabla .= "<tbody>";
		...
		$resultSendMail = mail( $emailTo, "Reportes diarios", "Consulta enviada: reportes diarios para todas las empresas con estado ER - En gestión, desde: $fechadesdeTexto hasta: $fechahastaTexto.
			Respuesta: código ".$resultadoJson->resultado->codigo." mensaje: ".$resultadoJson->resultado->error);
	...
	$resultSendMail = mail( $emailTo, "Reportes diarios", "No se obtuvo respuesta de ORMEN al consultar reportes diarios para todas las empresas con estado ER - En gestión, desde: $fechadesdeTexto hasta: $fechahastaTexto.");
	return $resultSendMail;
...
if ($filas != ""){
    ...
    $mensaje = '<html>
        ...
        <div align="center">'. $tabla . '</div></body></html>';


    $resultSendMail = mail( $emailTo, "Reportes diarios ".$fechadesdeTexto, $mensaje, $header);
    echo $resultSendMail;
    echo nl2br("\nEmail enviado\n");
