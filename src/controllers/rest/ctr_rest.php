...
class ctr_rest{
	...
	public function getNameClient($document){
		...
		$responseRest = json_decode(sendPetition::getNameClient($document));
	...
	public function nuevoCliente($rut, $data){
		$restController = new ctr_rest();
		$sendPetitionClass = new sendPetition();
		$responseGetToken = $restController->getToken();
		if($responseGetToken->result == 2){
			return json_decode($sendPetitionClass->nuevoCliente($rut, $data, $responseGetToken->token));
	...
	public function buscarCliente($rut, $textToSearch, $token){
		...
		$responseRest = json_decode(sendPetition::buscarCliente($rut, $textToSearch, $token));
	...
	public function buscarClienteDGI($textToSearch, $token){
		$response = new \stdClass();
		$response->listResult = array();
		$responseRest = json_decode(sendPetition::buscarClienteDGI($textToSearch, $token));
		if($responseRest->resultado->codigo == 200){
			$response->result = 2;
	...
	public function buscarClienteGenaroUyCedula($documento){
		$response = new \stdClass();
		$responseRest = sendPetition::prepareAndSendCurlGenaroUy("GET", "ci=".$documento);
	...
	public function buscarClienteGenaroUyNombre($nombre, $apellido){
		$response = new \stdClass();
		$response->listResult = array();
		$responseRest = sendPetition::prepareAndSendCurlGenaroUy("GET", "n=".$nombre."&a=".$apellido);
		if ( $responseRest == "ERROR"){
			$response->result = 1;
			$response->message = "No se ha encontrado cliente.";
		}
	...
	public function status($rut, $token){
		$response = new \stdClass();
		$responseRest = json_decode(sendPetition::status($rut, $token));
		if($responseRest->resultado->codigo == 200){
			$response->result = 2;
		}else{
			$response->result = 0;
	...
	public function exportacion($rut, $typeCall, $format, $dateFrom, $dateTo, $groupByCurrency, $includeReceipts, $type){
		$response = new \stdClass();

		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			...
			$responseRest = json_decode(sendPetition::exportacion($rut, $typeCall, $data, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
				$response->result = 2;
	...
	public function obtenerCotizacion($dateFrom, $dateTo, $typeCoin){
		...
		$responseRest = json_decode(sendPetition::obtenerCotizacion($dateFrom, $dateTo, $typeCoin));
	...
	public function consultarRut($rut, $rutBusiness, $token){
		...
		$responseRest = json_decode(sendPetition::consultarRut($rut, $rutBusiness, $token));
	...
	public function listarRecibidos($rut, $pageSize, $lastId, $dateFrom, $dateTo){
		...
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::listarRecibidos($rut, $pageSize, $lastId, $dateFrom, $dateTo, $responseToken->token));
			if(!isset($responseRest->resultado)){
	...
	public function listarEmitidos($rut, $pageSize, $lastId, $dateFrom, $branchCompany, $dateTo){
		$response = new \stdClass();

		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::listarEmitidos($rut, $pageSize, $lastId, $dateFrom, $dateTo, $branchCompany, $responseToken->token));
			if(!isset($responseRest->resultado)){
				if(sizeof($responseRest) > 0){
					$response->result = 2;
	...
	public function ping(){
		$response = new \stdClass();
		$responseRest = json_decode(sendPetition::ping());
		if($responseRest->resultado->codigo == 200){
			$response->result = 2;
	...
	public function signIn($rut, $user, $password){
		$response = new \stdClass();
		$responseRest = json_decode(sendPetition::login($rut, $user, $password, null));
		if($responseRest->resultado->codigo == 200){
			$response->result = 2;
	...
	public function consultarCFE($rut, $rutEmisor, $tipoCFE, $serieCFE, $numeroCFE, $formatFile){
		$response = new \stdClass();
		$restController = new ctr_rest();
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::consultarCFE($rut, $rutEmisor, $tipoCFE, $serieCFE, $numeroCFE, 1, $formatFile, $responseToken->token));
...
//					$restController->updateVoucherAnuladoDgi($tipoCFE, $serieCFE, $numeroCFE, $responseRest->isAnulado);
	...
	public function consultarCertificadoDigital($rut){
		...
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::consultarCertificadoDigital($rut, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
				unset($responseRest->resultado);
				$response->result = 2;
	...
	public function consultarCaes($rut){
		$response = new \stdClass();
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest =  json_decode(sendPetition::consultarCaes($rut, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
				$response->result = 2;
				unset($responseRest->resultado);
	...
	public function listarClientes($rut){
		...
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::listarClientes($rut, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
	...
	public function modificarCliente($rut, $document, $documentType, $name, $notificationMethod, $contacts){
		$response = new \stdClass();
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$data = array(
			...
			$responseRest = json_decode(sendPetition::modificarCliente($rut, $document, $data, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
				$response->result = 2;
	...
	public function consultarCliente($rut, $documento){
		$response = new \stdClass();

		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::consultarCliente($rut, $documento, $responseToken->token));
			if($responseRest->resultado->codigo == 200){
				$response->result = 2;
				unset($responseRest->resultado);
	...
	public function nuevoCFES($rut, $idEnvio, $tipoCFE, $fecha, $montosBrutos, $formaPago, $vencimiento, $receptor, $tipoMoneda, $detalles, $adenda, $sucursal){
		...
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$responseRest = json_decode(sendPetition::nuevoCFE($rut, $idEnvio, $tipoCFE, $fecha, $montosBrutos, $formaPago, $vencimiento, $receptor, $tipoMoneda, $detalles,$adenda, $sucursal,$responseToken->token));
	...
	public function nuevoCFE($rut, $tipoCFE, $fecha, $montosBrutos, $formaPago, $fechaVencimiento, $tipoMoneda, $exchangeRate, $detalle, $receptor, $indCobranza, $referencias, $adenda, $sucursal, $idBuy){
		$response = new \stdClass();
		...
		if($responseToken->result == 2){
			...
			$responseRest = json_decode(sendPetition::nuevoCFE($rut, $data, $responseToken->token));
			if($responseRest->resultado->codigo == 201){
				$response->result = 2;
				unset($responseRest->resultado);
				$response->cfe = $responseRest;
	...
	public function nuevoRecibo($rut, $tipoCFE, $fecha, $montosBrutos, $formaPago, $tipoMoneda, $usdValue, $detalle, $referencias, $receptor){
		$response = new \stdClass();
	...
	public function prepareContactDetail($contactType, $value){
		return sendPetition::getContactDetail($contactType, $value);
	}

	public function prepareReferenciasToSend($tipoCFE, $serieCFE, $numeroCFE, $indReferencia, $razon){
		return sendPetition::getReferenciasArray($tipoCFE, $serieCFE, $numeroCFE, $indReferencia, $razon);
	}

	public function prepareReceptorToSend($documento, $nombre, $direccion, $ciudad, $departamento, $pais){
		return sendPetition::getReceptorArray($documento, $nombre, $direccion, $ciudad, $departamento, $pais);
	}

	public function prepareDetalleToSend($indFact, $nomItem, $codItem, $descItem, $cantidad, $uniMedida, $precio){
		return sendPetition::getDetallesArray($indFact, $nomItem, $codItem, $descItem, $cantidad, $uniMedida, $precio);
	}
	...
	public function getBranchCompanyByRut ($rut){
		...
		$responseToken = ctr_rest::getToken();
		if($responseToken->result == 2){
			$company = sendPetition::getEmpresa($rut, $responseToken->token);
			$response->result = 2;
	...
	public function getToken(){
		...
		$responseGetToken = ctr_users::getUserInSesion();
	...
	public function updateVoucherAnuladoDgi($tipoCFE, $serieCFE, $numeroCFE, $isAnulado){
		$response = new stdClass();
		...
		$voucher = $emitedClass->getVoucherByTipoSerieNum($tipoCFE, $serieCFE, $numeroCFE, $business->idBusiness);
		if ( $voucher->result == 2 ){
			...
			if( $voucher->objectResult->isAnulado != $isAnulado ){
				...
				$response = $emitedClass->updateVoucherAnuladoByDgi($indice, $tipoCFE, $serieCFE, $numeroCFE, $isAnulado, $business->idBusiness);
			...
			$response->anular = false;
			return $response;
			error_log("ctr rest updateVoucherAnuladoDgi, no se encontro comprobante ".$tipoCFE." ". $serieCFE."-".$numeroCFE.", ".$business->idBusiness);
		}
