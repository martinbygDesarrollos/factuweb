...
class ctr_accounting{
	...
	public function countAllVouchersEmittedRest( $rut, $pageSize, $lastid, $dateFrom, $dateTo ){
		...
		if($responseCurrentSession->result == 2){
			$result = $emittedVoucherController->countAllVouchersEmittedRest( $rut, $pageSize, $lastid, $dateFrom, $dateTo );
	...
	public function exportSaleData( $rut, $pageSize, $dateFrom, $dateTo ){
		$userController = new ctr_users();
		...
		if($responseCurrentSession->result == 2){
			...
			$getEmitted = $restController->listarEmitidos($rut, $pageSize, null, $dateFrom, null, $dateTo);
			...
			if ( $getEmitted->result == 2 ){
				...
				foreach ($getEmitted->listEmitidos as $voucher) {
					...
					if ( $voucher->tipoCFE == 111 || $voucher->tipoCFE == 101 ){
						...
						$detailsData = $this->getDetailsFromVoucher( $rut, $voucher->tipoCFE, $voucher->serieCFE, $voucher->numeroCFE ); //pedir datos especificos del comprobante
	...
	public function getDetailsFromVoucher( $rut, $tipoCFE, $serieCFE, $numeroCFE ){
		$response = new \stdClass();
		$restController = new ctr_rest();
		$accountingController = new ctr_accounting();
		...
		$restResponse = $restController->consultarCFE($rut, null, $tipoCFE, $serieCFE, $numeroCFE, "application/json");
